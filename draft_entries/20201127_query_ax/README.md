...
セレクタ文字列 `aria/` prefix から始まると、デフォルトの CSS セレクタとしてではなく、ARIA Handler というクエリハンドラで検索されるように、カスタムハンドラが登録されている。このカスタムハンドラが登録されたのはは Puppeteer v5.4 から。
...
また、`'aria/Search[role="textbox"]'` のように、名前の後ろに `[role=<ロール名>]` を与えることで、Role で絞り込むことも可能。この例であれば「Search という名前の textbox」という意味になる。
...
昨今は CSS クラス名は自動生成されていたりすることも多く、ビルドをし直したら CSS クラス名が変わってしまって以前マッチしたセレクタが動かなくなることはザラだろう。
...
この考え方に乗っ取ると、Accessible Name や Role のような ARIA 情報を用いたクエリというのは、まさに「外部から見た振る舞いによる要素の特定手段」になり得る存在だ。
画面内の文言を利用するという意味では、従来でも XPath で DOM の text node を引っ掛ける程度のことはできるが、「支援技術から見てセマンティックな情報」というのは、それよりも数段強い。
...
1. `DOMDebugger.setEventListenerBreakpoint` で click や keydown を監視し、event 発生時に target node の Remote ID を取得
