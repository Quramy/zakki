...
上述の例のとおり、if 式や再帰関数定義、リストとパターンマッチなどをできるようにしている。基本的に OCaml のサブセットとなっている。
...
以降では、「どのような流れでプログラミング言語が実装されていくか」の概略を解説していきたいと思う。
...
言語機能のデザインが済んだら、設計した言語を抽象構文木で表すことを考える。
...
また、「プログラミング言語の意味」を考える上でも、評価機能は重要だ。

例えば、上記の評価機能は「二項演算子は左右それぞれの項を評価した結果を加算（もしくは乗算）する」という操作を行っている。「足算・掛算のプログラミング言語を作っているのだから当たり前でしょ？」と思うかもしれない。

だが、実際のところは逆なのだ。「操作の規則が二項演算という構文に意味を与えた」のだ。(Node.js における) 数値の演算を、`expr + expr` や `expr * expr` という BNF（で表現可能な AST）に割り振ったのは、僕の恣意的な選択の結果だ。小難しい言葉でいうと、「操作論的意味論として、評価規則を定めることでプログラミング言語にセマンティクスを与えた」といえる。

今回のエントリでは詳細の解説は行わないが、「変数に値を束縛する」「関数を適用する」なども、評価規則を明文化できて初めてセマンティクスを表現できた、ということができる。

### 4. コンパイル機能の実装

最後はいわゆるコンパイラバックエンドと呼ばれる部分。まずは「どのような環境で動作させたいか」に従って、出力対象を決める必要がある。PicoML は以下の理由で WebAssembly を選択している。
...
この例では、関数コードが以下に示す 7 バイトの列からなっているので、 `func body size` が 7 であり、関数の個数が 1 個であるため、7 + 1 (関数個数) + 1(func body size の分) = 9 となって、Code Section 全体のサイズが 9 バイトとというわけだ。
...
逆に、上記の関数コードのバイト列部分さえ生成すれば、後はその長さを数えて固定値のバイト列と組み合わせれば WASM を組み立てることができる。
...
今回のエントリでは、自作プログラミング言語 PicoML について、概要や実装したモチベーションと、簡単な計算式を WebAssembly にコンパイルする流れについて解説した。

他にも WebAssembly AST の取り扱いであったり、ML における環境やクロージャの概念をどのように WebAssembly の世界に落とし込むか、など紹介したいトピックはまだまだ色々あるのだけど、これらに言及しだすとキリが無くなってしまうため、また別に機会に取り上げていきたい。以下でソースコードを公開しているので、興味がある方は読んでみてください。
