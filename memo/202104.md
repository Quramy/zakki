...
module, memoryのparserだけ作ったところで、合わせてバイナリに出力する側も作り出す。

https://webassembly.studio でwat2wasmの結果も見つつ進める。

実装しながらspec を読んでいて初めて知ったが、 WASMのバイナリ表現では `Uint32` の符号化に LEB128という形式を用いている。 https://webassembly.github.io/spec/core/binary/values.html#binary-int
