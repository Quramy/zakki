...
とりあえず jest v27 で ts-jest を同時に上げないといけない問題をシュッとcloseしようとおもったところ、GitHub Actions上でtimeoutしてしまう。
`yarn jest` のCoverageがコンソールに出力されているので、恐らく後続の c8 によるE2E部分？ただ、同じbranchでローカルのNode.js v14.xで試すと普通に成功してしまう。
