...
https://code.claude.com/docs/ja/skills を読みながら PR レビューを行う skill を作成中。
そもそもの `/review` コマンドだと、自分が PR で指摘するようなポイントがあまり出てこないので、別の skill として作るようにしている。

過去に自分や他のメンバーが行った PR コメントを `gh` コマンドで取得させて、それをサマライズしたものを claude に作らせて SKILL.md から参照させるようなイメージ。
動作自体はするものの、狙ったコメントを出させるにようにするまでを考えると、チューニングやら色々が必要なのは明白なんだが、動作確認したらもう飽きつつある。
延々 .md をチマチマいじっては何回も回す、という作業、正直全然楽しくないんだよなぁ。
