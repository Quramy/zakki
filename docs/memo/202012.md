...
## TypeScript Library の Docs 自動生成について
...
## AST Explorer に Angular Template Parser 追加する
...
## Codelyzer から Angular ESLint のルール移行
...
## Relay と React Transport の関係について
...
## Can I Use Repo を Subscribe してみる
...
## Storybook の Viewport

Mobile first で開発する時は、以下あたりを設定しておくと捗りそう。
...
サブディスプレイとして使ってる iMac へのターゲットディスプレイ接続が完全にできなくなった。
ここ数日、接続不良が多かったなーという感はあるものの、一気に死んだなぁ。。。iMac 自体が死んだわけではないし、多分コネクタのどこかで断線してるんじゃないか説な気がするけど。。。

iMac と macbook、両方再起動したらなおった。。。そろそろちゃんとしたサブモニタ購入検討するか。。
...
Thymeleaf 動かすために Spring Boot の prj 作る。よくよく考えたら、build.gradle から書き始めるのは初めてだ。

横着してパッケージ切らずに Main class 置いたら起動できずにハマる。他の class とカチあってんのかな。

## Idea

やっぱり Vim バインドじゃないとつらいので IdeaVim 導入する

## Misc

Findy の Speakerdeck 連携が完了してた。連携直後は 0 スライド扱いでバグってんのかな？と思ったけど、バッチが回りきってないとかそういう系だったのかな。
「スライドタイトルに含まれるキーワード」ってとこ、stopwords だらけだ。

---

# 12/28

## React の `SuspenseList` について

あんまり何者かわかっていない `<SuspenseList>` の説明、普通に React の docs に載ってた。

https://ja.reactjs.org/docs/concurrent-mode-reference.html#suspenselist
...
GraphQL の `@stream` と併用することあるのかな？って思ってたけど、直接的には関係なさそう？

## GraphQL Client 自作

放置してた `@defer` の React Client 作り。
...
としていたけど、 Environment class は generic にするべきで、キャッシュのチェックと Promise の throw という行為は React（それも Concurrent Mode）specific な知識なので、hooks 側に寄せて Environment は素直に「query や fragment のデータを返す async function」に徹した方が綺麗だ。

```ts
function useQuery(query: any) {
  const env = useEnvironment();
  const result = env.readQuery(query);
  if (result) return result;
  throw env.executeQuery(query);
}
```

Store というか Cache 構築するのが一番つらいな、コレ。
defer 中かどうか、とかそういう時系列に沿った情報も cache に持たせないと駄目だよな？

上記で言うところの、 `readQuery` って部分が cache から同期的に fragment の情報を読み出す部分になるはずなんだけど、「その fragment 取得が進行中かどうか」を cache で管理しないといけないような？
「cache から取れない -> 取得中」っていう雑な判断をしちゃいけない気がする。
...
っていうクエリ書いたとして、雑に考えると、cache は以下のようになってるはずで、

```js
// 1st payload到達時のcache
[{ __typename: 'product', id: '100' }],

// 2nd payload到達時のcache
[{ __typename: 'product', id: '100', specialprice: null }],
```

どちらの状態であっても、`ProductPrice` fragment を cache から読み出そうとしたときに、 `specialPrice` が falsy となってしまうし。。。

```js
// 1st payload到達時のcache
[{ __typename: 'Product', id: '100', __deferred: true }],

// 2nd payload到達時のcache
[{ __typename: 'Product', id: '100', __deferred: false, specialPrice: null }],
```

みたいにするのがいいのかな。。。実際、`@defer` は複数使えるってことを考えると、`@defer(label: "ProductPrice")` と transform しておかないと区別できなさそう。

Operation を実行して返ってきた data を cache の形式に normalize するのはさして難しくないけど、 `readQuery` を実装しようとすると、この関数自身が resolver 的に振る舞わないといけないし、query や fragment に含まれている directive を解釈した上で、cache を読んでいかないといけないのか。。。

```graphql
query MyQuery($varFoo: Boolean!) {
  product(id: "100") {
    __typename
    id @include(if: true)
    name @skip(if: $varFoo)
    ...ProductPrice @defer(if: $varFoo, label: "ProductPrice")
  }
}
```

```js
// 2nd payload到達後のcache に対して、 `readQuery` がやること
//
// id は `@include(true)` で実行されているから、cacheになくてはいけない
// name は `@skip(true)` で実行されているから、cacheになくても問題ないのでOK
// `...ProductPrice` は `@defer` が完了しているため読み込める
[{ __typename: 'product', id: '100', specialPrice: null, __deferred: { ProductPrice: "resolved" }}],
```

これって intrinsic directive に対する処理を cache 上に再実装しろ、っていうことになる気がするけど、マジでそんなことしないといけないのか...？原理的にはできるんだけど、variable 解決したり、AST を traverse しながら directive 解決したりしていくの、大分面倒だな。。。

ここまでくると、 https://github.com/Quramy/graphql-streaming-example に片手間で実装するレベル超えてきてるかも。というか、ちゃんとテストコード書きながらじゃないと動く物作れる気がしないわ。。。

試しに Apollo Client の `readQuery` 実装調べてみたけど、やっぱり AST から変数解決とかしてるっぽいな。。。

https://github.com/apollographql/apollo-client/blob/29d41eb590/src/cache/inmemory/readFromStore.ts#L104-L136
...
bot がうんともすんとも言ってこなかった、typescript-eslint-language-service のレポに対して、integration の off/on をやりなおしたら、また bot から PR くるようになった。

今まで無視されてたの、どういう理由だったんだろう。。。？ :thinking_face:

## Misc

@masuP9 さんの a11y 記事 https://developers.cyberagent.co.jp/blog/archives/28310/

そろそろ今年の振り返り記事を書こうってなって、ふと思ったけど、「振り返り」ってまんま "look back" でいいのね。
...
git keygen plugni に bug ある issue（https://github.com/reg-viz/reg-suit/issues/336 ）来てたから見なきゃと思ったら、 @bokuweb が直してくれてた。ありがてぇ

## GraphQL Client 自作

Client 作るのであれば、名前空間を考えてみる。

- https://github.com/croquette は既に取られているが、 https://github.com/croquette-ql が空いている
- https://www.npmjs.com/~croquette も空いている。

とりあえず GitHub の organization を用意して TS monorepo 構成で開始してみる。

一昨日書いた、

> ここまでくると、 https://github.com/Quramy/graphql-streaming-example に片手間で実装するレベル超えてきてるかも。というか、ちゃんとテストコード書きながらじゃないと動く物作れる気がしないわ。。。

をやり始めてみたもの、悩みどころが多くてあまり気分が乗らない。

- id を select してない Object type について、cache の中で一意性を確保するにはどうすればいいのか？
  - Operation ID + query 内での location がを使えば一意になる？
- そもそも Operation ID をどうやって構成したらよいものか。
  - 案 1. Operation の text 表現 + variables を stringify したもの
  - 案 2. Operation の name.value + variables を stringify したもの
  - 案 3. 1 や 2 を hash 関数にかけた値(client side で動作させることを考えるとあまり現実的じゃない)
  - 案 4. 案 2 を key として、連番を振っておく。この map も serialize できる仕組みを用意しておく

## typescript-eslint-language-service

https://github.com/Quramy/typescript-eslint-language-service/issues/49#issuecomment-746049029

でコメント貰っていた「 "eslint:recommended" が resolve できていない」という件の対応をする。

最初、「plugin name conventional な文字列を resolve するのを全部こっちでやらないと駄目なのか！？」っと思い、適当に `"plugin:@typescript-eslint/recommended"` で試してみたところ、何の問題もなく動いてしまい（想定ではこれすら動かないと思っていた）ためにより混乱する。

ログでスタックトレースちゃんと吐くようにして、コード追ってみたところ、以下のように eslint パッケージ決め打ちで特殊処理してるのがわかった。逆にいうと、この 2 つだけちゃんと設定すればいい、ということなんだけど、何だこれって感じのコードだ。。。

https://github.com/eslint/eslintrc/blob/v0.2.2/lib/config-array-factory.js#L794-L813
...
とりあえず fix して release したけど、やっぱり internal な class に頼りまくっててホントつらいな、この plugin。
...
昨日に引き続き `readQuery` 部分の実装。シンプルな Field Selection のパターンは大体できた。

Fragment の実装入る前に、Inline Fragment のときの整理がいる？

```graphql
query {
  search(query: "graphql", first: 10, type: USER) {
    nodes {
      __typename
      ... on Repository {
        __typename
...
例えば、上記は GitHub v4 API で valid な Query だけど、 `RepositoryOwner` は interface なので、実際の `__typename` としては、 `User` や `Organization` といった `RepositoryOwner` を implement した Object の名前が入ってくる。query だけを見ても、cache の record 名が確定できなさそうな気がするけど、大丈夫かコレ？
Fragment matcher 的なものを用意して、Schema から生成させるか、query の結果を cache に書き込むときに、matching 情報を更新していけばいける...?

## Misc

`lerna run` と `lerna exec` を覚え違いしてた。 各 package の script を一斉に走らせるのは `lerna run` の方だった。
