...
※ Next.js Conf はとくにみていない

Cache Components が目玉扱いなのかな。
PPR を包含しているというか、PPR を前提とした機能という感じかしら。
Data Cache, Full Route Cache レイヤの新しいバージョンを置き換えるもの、というイメージだけど `use cache` など自分では全然触ってないのでまだよくわかってない。
...
v10 で Storybook 本体は ESM 版のみの提供となる（今までは CJS / ESM のデュアルバンドルだった）。

https://storybook.js.org/blog/storybook-is-going-esm-only/

試しに手元で upgrade してみたものの、 Jest で `composeStory` している都合で Jest 側で transform 失敗のエラーとなってしまう。
`@swc/jest` の設定の問題だと思うが、回避方法がわからないため一旦 v9 系に戻した。
