...
Vite が Rolldown に乗り換えが完了されると、またもや parser が切り替わることになる。
というのは、Rolldown が依存している parser は OXC であるため。

なんというか、ESTree に連なる acorn であったり ESLint の終焉を感じるなー。Turbopack は swc だし、prettier のレイヤについても Biome が出てきてるし、Vite は OXC に依存することになるわけで、どの Crate かというのはさておいて、少なくとも js / ts の AST を parse するという意味では JavaScript による実装が使われることは今後ないのかな。
