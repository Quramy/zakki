...
## React の Suspense for Data Fetching が Throw promise を辞めるかも
...
## RHF における error focus

Validation Error 発生時に、「先頭のエラーメッセージにスクロールさせる」というありがち要件。
一見、RHF 単独でできそうに思えるが、あれはあくまで当該の input に対する focus だし、Chrome 以外だと focus されることと Viewport にその要素が含まれることは別だったりする。

エラーメッセージが複数あるときに真面目に `useRef` で参照扱うのは結構面倒なので、つい `querySelector` でやってしまうのよなぁ。。。

```ts
docuement.querySelector("[role='alert']")?.scrollIntoView();
```
